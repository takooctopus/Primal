...
namespace primal::graphics::d3d12::gpass {
	namespace {
		...
		bool create_buffers(math::u32v2 size) {
			...
			desc.Alignment = 0; // 对齐 64KB (只有 MSAA 4MB)
			...
			desc.MipLevels = 0;	// 为所有mip level 都准备空间
			desc.SampleDesc = { 1,0 }; // 不使用多重采样
	...
	bool initialize() {
		//创建缓冲区
		//创建属于Gpass的 root_signature 和 对应的 Pipeline State Object
		return create_buffers(initial_dimensions) && create_gpass_pso_and_root_signature();
	}

	void shutdown() {
		gpass_main_buffer.release();	//释放主缓冲
		gpass_depth_buffer.release();	//释放深度缓冲
		dimensions = initial_dimensions;	// 重置维度
	...
	{
		// 深度缓冲里面不需要输出颜色【将深度设置成z轴】
		const D3D12_CPU_DESCRIPTOR_HANDLE dsv{ gpass_depth_buffer.dsv() };	//获取深度模板描述信息句柄
		cmd_list->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 0.f, 0, 0, nullptr);
		cmd_list->OMSetRenderTargets(0, nullptr, 0, &dsv);	//只绑定深度
	}

	void set_render_targets_for_gpass(id3d12_graphics_command_list* cmd_list) {
		const D3D12_CPU_DESCRIPTOR_HANDLE rtv{ gpass_main_buffer.rtv(0) };	//获取render_target
		const D3D12_CPU_DESCRIPTOR_HANDLE dsv{ gpass_depth_buffer.dsv() };	//获取深度模板描述信息句柄
		cmd_list->ClearRenderTargetView(rtv, clear_value, 0, nullptr);
		cmd_list->OMSetRenderTargets(1, &rtv, 0, &dsv);	//绑定rtv和dsv
	}
}
