...
namespace primal::graphics::d3d12::fx {
	namespace {
		...
		bool create_fx_pos_and_rooty_signature() {
			assert(!fx_root_sig && !fx_pso);
			
			// 创建Post-process FX signature
			...
			NAME_D3D12_OBJECT(fx_pso, L"Post-process FX State Object");
	...
	bool initialize()
	{
		return create_fx_pos_and_rooty_signature();
	}
	void shutdown()
	{
		core::release(fx_pso);
